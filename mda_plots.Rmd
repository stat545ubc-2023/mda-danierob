...
__Data Exploration 2:__
```{r}
# Excercise 1: Determining the distribution of the numeric variable (diameter) to start wrapping my head around the data and considering what kind of relationships may be present and what analysis might be required.
```
```{r}
...
```{r}
# Following excercise 5, we will use filter() to remove the outliers (greater than 100 inches):
...
```{r}
# Following exercise 4: Assess the relationship between two variables. We'll look at diameter and neighborhood.
...
```
```
Upon firest glance, average DBH appears to be quite similair among the different neighborhoods, thought there is some clear variation (downtown has notably smaller trees).
